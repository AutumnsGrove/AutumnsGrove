    ...
    if (field in body) {
      ...
      if (field === "brief_summary") {
        ...
        if (value.length > MAX_BRIEF_SUMMARY_LENGTH) {
          throw error(
            400,
            `brief_summary exceeds maximum length of ${MAX_BRIEF_SUMMARY_LENGTH} characters`,
          );
        }
      }
      ...
      if (field === "detailed_timeline") {
        ...
        if (value.length > MAX_DETAILED_TIMELINE_LENGTH) {
          throw error(
            400,
            `detailed_timeline exceeds maximum length of ${MAX_DETAILED_TIMELINE_LENGTH} characters`,
          );
        }
      }
  ...
  try {
    ...
    if (kv) {
      try {
        ...
        console.log(
          `Cache invalidated for date ${date}: ${cachePatterns.length} keys`,
        );
      } catch (e) {
