<script>
	...
	import { toast } from '@autumnsgrove/grove-engine/components/ui/toast';
...
<style>
	...
	:global(.dark) .timeline-header p {
		color: var(--color-text-muted-dark, var(--light-text-muted));
	}
	/* Activity Loading */
	.activity-loading {
		display: flex;
	...
	.card-header {
		...
		align-items: center;
		padding: 1rem;
		border-bottom: 1px solid var(--light-text-very-light);
	}
	...
	.day-charts {
		...
		margin: 0.5rem 0;
		padding: 0.5rem 0;
		border-top: 1px dashed var(--light-text-very-light);
	}
