  ...
  * . . * . . * . *
  ...
  async function runAnalysis(action = "all") {
    ...
    try {
      ...
      analysisError = "Could not connect to AI service";
    } finally {
      isAnalyzing = false;
    }
  }
  ...
  {#if isOpen && !isMinimized}
    ...
    >
      ...
      <div class="model-selector">
        <label>
          <input type="radio" bind:group={selectedModel} value="haiku" />
          <span>haiku (fast)</span>
        </label>
        <label>
          <input type="radio" bind:group={selectedModel} value="sonnet" />
          <span>sonnet (deeper)</span>
        </label>
      </div>
        ...
        <div class="results" transition:slide>
          ...
          <div class="usage-info">
            {#if results.tokensUsed}
              <span>tokens: {results.tokensUsed}</span>
...
<style>
  ...
  .model-selector input[type="radio"] {
    accent-color: var(--accent, #8bc48b);
  }
  ...
  .tab.active {
    color: var(--accent, #8bc48b);
    border-bottom-color: var(--accent, #8bc48b);
  }
  ...
  .suggestion.severity-style {
    border-left-color: var(--accent, #8bc48b);
  }
