<script>
  ...
  // CDN Image Picker state
  let imagePicker = $state({
    open: false,
    images: [],
    loading: false,
    error: null,
  ...
  const slashCommands = [
    ...
    { id: "table", label: "Table", insert: "| Header | Header | Header |\n|--------|--------|--------|\n| Cell   | Cell   | Cell   |\n| Cell   | Cell   | Cell   |\n" },
    { id: "link", label: "Link", insert: "[](url)", cursorOffset: 1 },
    { id: "image", label: "Image", insert: "![alt](url)", cursorOffset: 2 },
    { id: "gallery", label: "Gallery (CDN Images)", insert: "", isAction: true, action: () => openImagePicker() },
  ...
  function closeImagePicker() {
    ...
    imagePicker.filter = "";
    imagePicker.cursor = null;
    imagePicker.error = null;
  }

  async function loadGalleryImages(append = false) {
    imagePicker.loading = true;
    imagePicker.error = null;

    try {
      const params = new URLSearchParams();
      if (imagePicker.filter) params.set("prefix", imagePicker.filter);
      if (append && imagePicker.cursor) params.set("cursor", imagePicker.cursor);
      params.set("limit", "30");
      ...
      if (append) {
        imagePicker.images = [...imagePicker.images, ...filteredImages];
      } else {
        imagePicker.images = filteredImages;
      }
      imagePicker.cursor = data.cursor;
      imagePicker.hasMore = data.truncated;
    } catch (err) {
      imagePicker.error = err.message;
    } finally {
      imagePicker.loading = false;
    }
  }
  ...
  function insertImageFromGallery(image) {
    const filename = image.key.split("/").pop();
    const markdown = `![${filename}](${image.url})`;
...
>
  ...
  <div class="toolbar">
    ...
    <div class="toolbar-group">
      ...
      <button
        ...
        title="Table"
        disabled={readonly}
      >[ta<span class="key">b</span>le]</button>
    </div>
    ...
    <div class="toolbar-group">
      <button
        ...
        title="Insert image from CDN gallery"
        disabled={readonly}
      >[<span class="key">g</span>allery]</button>
    </div>
  ...
  <div class="shortcut-hints">
    ...
    <span class="shortcut-item"><span class="key">B</span> bold</span>
    <span class="shortcut-sep">·</span>
    <span class="shortcut-item"><span class="key">I</span> italic</span>
    <span class="shortcut-sep">·</span>
    <span class="shortcut-item"><span class="key">S</span> save</span>
    <span class="shortcut-sep">·</span>
    <span class="shortcut-item"><span class="key">K</span> palette</span>
    <span class="shortcut-sep">·</span>
    <span class="shortcut-item"><span class="key">/</span> commands</span>
  </div>
...
<!-- CDN Image Picker Modal -->
{#if imagePicker.open}
  <div class="image-picker-overlay" onclick={closeImagePicker}>
    <div class="image-picker-modal" onclick={(e) => e.stopPropagation()}>
      ...
      <div class="image-picker-filter">
        <input
          type="text"
          bind:value={imagePicker.filter}
          placeholder="Filter by folder (e.g., blog/, recipes/)..."
          onkeydown={(e) => e.key === "Enter" && filterGalleryImages()}
        />
        <button type="button" class="filter-btn" onclick={filterGalleryImages}>
          [filter]
        </button>
      </div>

      <div class="image-picker-body">
        {#if imagePicker.loading && imagePicker.images.length === 0}
          <div class="image-picker-loading">Loading images...</div>
        {:else if imagePicker.error}
          <div class="image-picker-error">{imagePicker.error}</div>
        {:else if imagePicker.images.length === 0}
          <div class="image-picker-empty">No images found</div>
        {:else}
          <div class="image-picker-grid">
            {#each imagePicker.images as image}
              <button
                type="button"
              ...
              >
                <img
                  ...
                  loading="lazy"
                />
                <span class="image-picker-name">{image.key.split("/").pop()}</span>
              </button>
            {/each}
          </div>

          {#if imagePicker.hasMore}
            <div class="image-picker-load-more">
              <button
                type="button"
                class="load-more-btn"
              ...
              >
                {#if imagePicker.loading}[loading...]{:else}[load more]{/if}
              </button>
            </div>
          {/if}
        {/if}
...
<style>
  ...
  /* Gallery Button */
  .toolbar-btn.gallery-btn {
    color: #c8a8e8;
  }

  .toolbar-btn.gallery-btn:hover {
    color: #dac0f0;
  }

  .toolbar-btn.gallery-btn .key {
    color: #dac0f0;
  }
  ...
  .image-picker-filter input:focus {
    outline: none;
    border-color: #c8a8e8;
  }
  ...
  .filter-btn:hover {
    background: rgba(200, 168, 232, 0.1);
    border-color: #c8a8e8;
  }
