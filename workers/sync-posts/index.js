...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === '/sync' && request.method === 'POST') {
      ...
      } else if (url.pathname === '/sync-recipes' && request.method === 'POST') {
        return await handleSyncRecipes(request, env, corsHeaders);
      } else if (url.pathname === '/recipes' && request.method === 'GET') {
        return await handleGetRecipes(env, corsHeaders);
      } else if (url.pathname.startsWith('/recipes/') && request.method === 'GET') {
        const rawSlug = url.pathname.split('/')[2];
        const slug = normalizeSlug(rawSlug);
        if (!slug || !isValidSlug(slug)) {
          return new Response(JSON.stringify({ error: 'Invalid slug format' }), {
        ...
        return await handleGetRecipe(slug, env, corsHeaders);
      } else {
...
async function handleSyncRecipes(request, env, corsHeaders) {
  // Check rate limiting
  const rateLimitResult = await checkRateLimit(request, env);
  if (rateLimitResult.limited) {
    return new Response(JSON.stringify({ error: 'Rate limit exceeded' }), {
  ...
  const MAX_RECIPES_PER_SYNC = 100;
  if (recipes.length > MAX_RECIPES_PER_SYNC) {
    return new Response(JSON.stringify({
      error: `Too many recipes in single sync request. Maximum: ${MAX_RECIPES_PER_SYNC}, received: ${recipes.length}.`
    }), {
      status: 413,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });
  ...
  for (const recipe of recipes) {
    ...
    const normalizedSlug = normalizeSlug(recipe.slug);

    if (!normalizedSlug || !isValidSlug(normalizedSlug)) {
      return new Response(JSON.stringify({
        error: `Invalid slug format in payload: "${originalSlug}"`
      }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    ...
    if (!recipe.content || typeof recipe.content !== 'string') {
      return new Response(JSON.stringify({ error: `Missing content for recipe: ${recipe.slug}` }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }

    if (recipe.content.length > MAX_CONTENT_SIZE) {
      return new Response(JSON.stringify({ error: `Content too large for recipe: ${recipe.slug}` }), {
        status: 413,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
  ...
  // Batch fetch existing recipes
  const existingRecipesQuery = await env.DB.prepare(
    `SELECT slug, file_hash FROM recipes WHERE slug IN (${recipes.map(() => '?').join(',')})`
  ).bind(...recipes.map(r => r.slug)).all();

  const existingRecipesMap = new Map(
    existingRecipesQuery.results.map(r => [r.slug, r.file_hash])
  );
  ...
  for (const recipe of recipes) {
    try {
      const fileHash = await generateHash(recipe.content);
      const existingHash = existingRecipesMap.get(recipe.slug);
      ...
      const { data, content: markdownContent } = matter(recipe.content);
      const htmlContent = marked.parse(markdownContent);
      ...
      if (title.length > MAX_TITLE_LENGTH) {
        results.errors.push({ slug: recipe.slug, error: `Title too long` });
        continue;
      }

      if (description.length > MAX_DESCRIPTION_LENGTH) {
        results.errors.push({ slug: recipe.slug, error: `Description too long` });
        continue;
      }

      const tagsJson = JSON.stringify(data.tags || []);
      const timestamp = new Date().toISOString();
      const recipeDate = data.date instanceof Date
        ? data.date.toISOString()
        : (data.date || new Date().toISOString());
      ...
      if (existingHash) {
        batchStatements.push(
          env.DB.prepare(`
            UPDATE recipes
            SET title = ?, date = ?, tags = ?, description = ?, url = ?,
                markdown_content = ?, html_content = ?, file_hash = ?,
                last_synced = ?, updated_at = CURRENT_TIMESTAMP
            WHERE slug = ?
          `).bind(
            title, recipeDate, tagsJson, description, url,
            recipe.content, htmlContent, fileHash, timestamp, recipe.slug
          )
        );
        results.details.push({ slug: recipe.slug, action: 'updated' });
      } else {
        batchStatements.push(
          env.DB.prepare(`
            INSERT INTO recipes (
              slug, title, date, tags, description, url,
              markdown_content, html_content, file_hash, last_synced
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).bind(
            recipe.slug, title, recipeDate, tagsJson, description, url,
            recipe.content, htmlContent, fileHash, timestamp
          )
        );
        results.details.push({ slug: recipe.slug, action: 'created' });
      }

      pendingSyncCount++;
    } catch (error) {
      console.error(`Error syncing recipe ${recipe.slug}:`, error);
      results.errors.push({
        slug: recipe.slug,
        error: env.ENVIRONMENT === 'production' ? 'Sync failed' : error.message
      });
  ...
  try {
    const dbSlugs = await env.DB.prepare('SELECT slug FROM recipes').all();
    const syncSlugs = new Set(recipes.map(r => r.slug));
    ...
    if (slugsToDelete.length > 0) {
      const deleteStatements = slugsToDelete.map(slug =>
        env.DB.prepare('DELETE FROM recipes WHERE slug = ?').bind(slug)
      );
      ...
      slugsToDelete.forEach(slug => {
        results.details.push({ slug, action: 'deleted' });
      });
    }
  } catch (error) {
    console.error('Error handling deleted recipes:', error);
    results.errors.push({
      action: 'cleanup',
      error: env.ENVIRONMENT === 'production' ? 'Cleanup failed' : error.message
    });
...
async function handleGetRecipes(env, corsHeaders) {
  try {
    const { results } = await env.DB.prepare(`
      SELECT slug, title, date, tags, description, url, last_synced, created_at, updated_at
      FROM recipes
      ORDER BY date DESC
    `).all();
...
async function handleGetRecipe(slug, env, corsHeaders) {
  try {
