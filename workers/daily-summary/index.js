...
async function trackAIUsage(
  ...
  try {
    ...
    `,
      ...
      .bind(today, provider, model, inputTokens, outputTokens, cost)
      .run();
...
function getCorsHeaders(env) {
  return {
    "Access-Control-Allow-Origin":
      env.ALLOWED_ORIGIN || "https://autumnsgrove.com",
...
export default {
  /**
   * Queue consumer handler (required for queue bindings)
   * Currently a placeholder - queues kept for future background job processing
   */
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/health") {
        return new Response(
          JSON.stringify({ status: "ok", defaultProvider: DEFAULT_PROVIDER }),
          {
            headers: corsHeaders,
          },
        );
      ...
      if (url.pathname === "/usage" && request.method === "GET") {
        ...
        for (const row of usage.results) {
          totals.byProvider[row.provider] =
            (totals.byProvider[row.provider] || 0) + row.estimated_cost_usd;
          totals.byModel[row.model] =
            (totals.byModel[row.model] || 0) + row.estimated_cost_usd;
        ...
        const dailyCosts = {};
        for (const row of usage.results) {
          dailyCosts[row.usage_date] =
            (dailyCosts[row.usage_date] || 0) + row.estimated_cost_usd;
        }
      ...
      if (url.pathname === "/backfill" && request.method === "POST") {
        ...
        if (!startDate) {
          return new Response(
            JSON.stringify({ error: "Missing start date parameter" }),
            {
              status: 400,
              headers: corsHeaders,
            },
        ...
        if (daysDiff > maxDays) {
          return new Response(
            JSON.stringify({
              error: `Maximum ${maxDays} days allowed per request`,
            }),
            {
              status: 400,
              headers: corsHeaders,
      ...
      if (url.pathname === "/latest" && request.method === "GET") {
        ...
        return new Response(
          JSON.stringify(latest || { message: "No summaries yet" }),
          {
            headers: corsHeaders,
          },
        );
