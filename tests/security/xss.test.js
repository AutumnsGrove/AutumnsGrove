...
// Mock the sanitization utilities to use isomorphic-dompurify in Node environment
vi.mock("@autumnsgrove/groveengine/utils");
...
describe("XSS Protection", () => {
  describe("HTML Sanitization", () => {
    ...
    it("should handle HTML entities without executing them", () => {
      ...
      const hasNoScript =
        !clean.includes("<script>") || clean.includes("&lt;script&gt;");
    ...
    it("should handle multiple script tags", () => {
      const malicious =
        "<p>Start</p><script>alert(1)</script><p>Middle</p><script>alert(2)</script><p>End</p>";
      const clean = sanitizeHTML(malicious);
    ...
    it("should handle mixed dangerous and safe content", () => {
      const malicious =
        "<p>Safe text</p><script>alert(1)</script><strong>More safe text</strong>";
      const clean = sanitizeHTML(malicious);
    ...
    it("should preserve safe HTML while removing dangerous parts", () => {
      const mixed =
        '<h1>Title</h1><p>Paragraph</p><script>alert(1)</script><a href="http://example.com">Link</a>';
  ...
  describe("SVG Sanitization", () => {
    ...
    it("should remove event handlers from SVG elements", () => {
      const malicious =
        '<svg><circle onclick="alert(1)" cx="50" cy="50" r="40" /></svg>';
      const clean = sanitizeSVG(malicious);
